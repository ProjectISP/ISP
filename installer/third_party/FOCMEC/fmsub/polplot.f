C+
	SUBROUTINE POLPLOT(TD,RADIUS,UPPER)
C
C     Modified prplot.f: leaves out ratios. RATPLT does ratios.
C	Reads in polarity data from a file, plots it
...
C         L = same as <
C         R = same as > (Earlier versions used R for SV/P ratio)
...
C	  with a duplicate line with a E in the
C	  symbol position.  The error will then be flagged
C	  as a square superimposed on the polarity.
C-
	CHARACTER*1 SVSHP,CODE
      character*24 td
	CHARACTER*80 FILENA, COM, CVALUE, DUMMY
	CHARACTER*40 TITLE
	LOGICAL TRUTH,PIMP,PEMERG,PGUESS,SHIMP,SHGUES,SHEMRG
