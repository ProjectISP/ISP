...
c
      ...
      write(10,200)nk,ncr,ric,roc,a0,(i,r(i),vp(i),vs(i),i=1,nk)
 200  format(/1x,2i5,3f10.2/(i5,f10.2,2f10.4))
      pn=vs(1)
      xn=1/r(1)
      tn=pn*xn
      do 1 i=1,nk
...
c   Loop over each layer (between two discontinuities).
 1    r0=r1
      r1=.001
      if(i.gt.0) r1=rd(i)
      if(abs(r1-rocn).lt.abs(roc-rocn)) roc=r1
      if(abs(r1-ricn).lt.abs(ric-ricn)) ric=r1
      if(r1.lt.roc) fluid=.true.
      l=(r0-r1)/dr-.5
      dx=(r0-r1)/(l+1)
      print *,'r0 r1 roc ric fluid l dx =',r0,r1,roc,ric,fluid,l,dx
...
c   Make P and S velocity the same if we are in a fluid.
 2    if(fluid) vs(n)=vp(n)
      print *,'top i n r vp vs =',i,n,r(n),vp(n),vs(n)
...
c   Make P and S velocity the same if we are in a fluid.
      if(fluid) vs(n)=vp(n)
      print *,'bot i n r vp vs =',i,n,r(n),vp(n),vs(n)
...
c
 ...
 4    continue
      if(l.eq.2) then
         ps0=ps(1)-.25*du
         x0=xmod(ps0,nph,1,nr,r2)
         write(10,*)'    extra  =',ps0,x0
         if((xs(2)-x0)*(x0-xs(1)).le.0.) then
            ps(3)=ps(2)
            xs(3)=xs(2)
            ps(2)=ps0
            xs(2)=x0
...
c
 ...
 14   continue
      kk=l
      write(10,*)'off-the-end',kk,x0,xs(kk),xs(kk-1)
 15   mm=kk
...
c
c                                                     -rpb
c  August 2008: Original code did not work for building ak135 on the Sun f95
c  compiler because of an "arithmetic exception (core dump)' in the iteration
c  loop.  I followed George Hellfrich's lead by declaring some variables
c  used in that loop double precision, and now it works as on other compilers.
c  jas/vt.
c
      save 
      include 'limits.inc'
      ...
      common/pgridc/delx(2),dpmax(2),drmax(2),pp(nsl1,2),xx(nsl1,2),
     1 rr(nsl1,2)
C     common/findc/x(20),y(20),m
      double precision x(20),y(20),fx(20)
      double precision ps0,ps1,xi,yi,dy
c
      m=1
...
cgh      yi=xmod(xi,nph,1,nr,r0)-xg
      fx(m)=xmod(real(xi),nph,1,nr,r0)
      yi=fx(m)-xg
...
c
 ...
 4    continue
      ...
      if(code(lc,nph)(2:).eq.'PKP'.and.code(lc-1,nph)(2:).eq.'Pn')
     1 code(lc-1,nph)(3:)=' '
      if(code(lc,nph)(2:).eq.'SKS'.and.code(lc-2,nph)(2:).eq.'Sn')
     1 code(lc-2,nph)(3:)=' '
