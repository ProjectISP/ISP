...
class mouse:
    ...
    def create(self, paz, length, dt, onset, typ=1, demean=True, integrate=True):
        ...
        for i in range(1, int(
                nmax / 2) + 1):  # negative frequencies nmax/2+1 .. nmax-1 are complex conjugate of positive frequencies
            f = i * df
            if (typ == 2):
                ...
                mys[i] = (1. / dt) / (
                            2. * np.pi * 1j * f) ** 2.  # input velocity= linear  (i.e. input accel = step); mouse I
    ...
    def demean(self):
        last = int(
            self.onset / self.dt - 10)  # take 10 samples less to avoid possible noc-causal blend of the mouse onset
        sum = 0
        for i in range(last):
            sum += self.mouse[i]
        mean = sum / last
    ...
    def invert(self, N, E, Z, shift):
        ...
        for i in range(fit_t0, fit_t1):
            ms_N += mys[i + shift - on].real * N[
                i]  # \frac{\sum_i m_i s_i^E}{\sum_i m_i s_i^N} = \frac{\sin \phi}{\cos \phi} = \tg \phi
        ...
        for i in range(fit_t0, fit_t1):
            fit_nom += (N[i] - mys[i + shift - on].real * a * cos(ph) * cos(th)) ** 2 + (
                        E[i] - mys[i + shift - on].real * a * sin(ph) * cos(th)) ** 2 + (
                                   Z[i] - mys[i + shift - on].real * a * sin(th)) ** 2
    ...
    def plot(self, st, outfile='', distance=5e3, ylabel='', xmin=100, xmax=300, legend=False, yaxes=True,
        ...
        if type(st) == stream.Stream:
            for tr in st:
                ...
                if ch == 'E':
                    m = plt.plot(t2,
                                 np.real(self.mouse[samp_0:samp_1]) * self.amplitude * sin(self.phi) * cos(self.theta) +
                                 shifts[ch], colors[ch] + '--')
                elif ch == 'N':
                    m = plt.plot(t2,
                                 np.real(self.mouse[samp_0:samp_1]) * self.amplitude * cos(self.phi) * cos(self.theta) +
                                 shifts[ch], colors[ch] + '--')
                elif ch == 'Z':
                    m = plt.plot(t2, np.real(self.mouse[samp_0:samp_1]) * self.amplitude * sin(self.theta) + shifts[ch],
                                 colors[ch] + '--', label='simulated')
            ...
            plt.title(
                title.format(net=tr.stats.network, sta=tr.stats.station, date=tr.stats.starttime.strftime("%Y-%m-%d"),
