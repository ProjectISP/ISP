...
output_files_path = config_object["PATHS"]["output_files_path"]  # Ficheros Pickle (salida del proceso anterior)
stack_files_path = config_object["PATHS"]["stack_files_path"]  # Ficheros MSEED (salida del proceso anterior)
channel = config_object["OPTIONS"]["channel"]  # BHZ, BHN, BHE
year = int(config_object["OPTIONS"]["year"])
check_nan = True if (config_object["OPTIONS"]["check_nan"] == "True") else False
# Se crea la carpeta si no existe:
if not os.path.exists(stack_files_path):
    os.makedirs(stack_files_path)

# Ficheros de datos
pickle_files = [pickle_file for pickle_file in os.listdir(output_files_path) if channel in pickle_file]
...
for i, file_i in enumerate(pickle_files):

    for j, file_j in enumerate(pickle_files):

        print("(i=" + str(i) + ",j=" + str(j) + ") -> (" + file_i + "," + file_j + ")")

        with open(os.path.join(output_files_path, file_i), 'rb') as h_i, open(os.path.join(output_files_path, file_j),
                                                                              'rb') as h_j:
            ...
            if (len(date_list_file_i) > 0 and len(date_list_file_j) > 0):
                ...
                if j >= i:
                    ...
                    stats['starttime'] = UTCDateTime(year=year, julday=common_dates_list[0], hour=0, minute=0)
                    st = Stream([Trace(data=c_stack, header=stats)])
                    # Nombre del fichero = XT.STA1_STA2.BHZ
                    filename = file_i[:2] + "." + file_i[2:6] + "_" + file_j[2:6] + "." + channel
                    st.write(os.path.join(stack_files_path, filename), format='MSEED')
